...
import { ArrowLeft, X, Building2, Home, Eye, EyeOff } from 'lucide-react';
...
const UNIT_FEATURE_LABELS = ['Sacada', 'Sacada com Churrasqueira', 'Sacada Integrada', 'Lavabo', 'Área de Serviço', 'Varanda', 'Mobiliado', 'Ar Condicionado', 'Energia Solar'];
const BUILDING_FEATURE_LABELS = ['Piscina', 'Academia', 'Playground', 'Churrasqueira', 'Salão de Festas', 'Portaria 24h', 'Elevador', 'Jardim'];
const ALL_KNOWN_FEATURES = [...UNIT_FEATURE_LABELS, ...BUILDING_FEATURE_LABELS];
...
const EditProperty = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    is_pre_launch: false,
    olx_status: null as string | null,
    olx_error_message: null as string | null,
  ...
  useEffect(() => {
    async function loadProperty() {
      ...
      try {
        ...
        const unitFeatures = Array.isArray((data as any).unit_features) ? (data as any).unit_features : [];
        const buildingFeatures = Array.isArray((data as any).building_features) ? (data as any).building_features : [];

        // Determine feature flags from either new arrays or legacy features array
        const allUnitFeatures = unitFeatures.length > 0 ? unitFeatures : features;
        const allBuildingFeatures = buildingFeatures.length > 0 ? buildingFeatures : features;
        ...
        setFormData({
          ...
          lavabos: (data as any).lavabos ? String((data as any).lavabos) : '',
          suites: data.suites ? String(data.suites) : '',
          garage_spaces: data.garage_spaces ? String(data.garage_spaces) : '',
          ...
          development_name: (data as any).development_name ?? '',
          development_description: (data as any).development_description ?? '',
          apartment_number: (data as any).apartment_number ?? '',
          show_apartment_details: Boolean((data as any).show_apartment_details),
          ...
          is_pre_launch: Boolean((data as any).is_pre_launch),
          olx_status: data.olx_status ?? null,
          olx_error_message: data.olx_error_message ?? null,
          accepts_exchange: Boolean(data.accepts_exchange),
          hide_address: Boolean(data.hide_address),
          // Unit features
          has_sacada: allUnitFeatures.includes('Sacada'),
          has_sacada_churrasqueira: allUnitFeatures.includes('Sacada com Churrasqueira'),
          has_sacada_integrada: allUnitFeatures.includes('Sacada Integrada'),
          has_lavabo: allUnitFeatures.includes('Lavabo'),
          has_area_servico: allUnitFeatures.includes('Área de Serviço'),
          has_varanda: allUnitFeatures.includes('Varanda'),
          has_furnished: allUnitFeatures.includes('Mobiliado'),
          has_air_conditioning: allUnitFeatures.includes('Ar Condicionado'),
          has_solar_energy: allUnitFeatures.includes('Energia Solar'),
          customUnitFeatures: (unitFeatures.length > 0 ? unitFeatures : features).filter((f: string) => !ALL_KNOWN_FEATURES.includes(f)),
          // Building features
          has_pool: allBuildingFeatures.includes('Piscina'),
          has_gym: allBuildingFeatures.includes('Academia'),
          has_playground: allBuildingFeatures.includes('Playground'),
          has_barbecue: allBuildingFeatures.includes('Churrasqueira'),
          has_party_room: allBuildingFeatures.includes('Salão de Festas'),
          has_concierge: allBuildingFeatures.includes('Portaria 24h'),
          has_elevator: allBuildingFeatures.includes('Elevador'),
          has_garden: allBuildingFeatures.includes('Jardim'),
          customBuildingFeatures: (buildingFeatures.length > 0 ? buildingFeatures : []).filter((f: string) => !ALL_KNOWN_FEATURES.includes(f)),
        });
      } catch (error) {
        toast({ title: "Erro ao carregar imóvel", description: "Não foi possível carregar os dados do imóvel.", variant: "destructive" });
      } finally {
        setLoading(false);
      }
    }
  ...
  const removeUnitFeature = (f: string) => setFormData({ ...formData, customUnitFeatures: formData.customUnitFeatures.filter(x => x !== f) });
  ...
  const removeBuildingFeature = (f: string) => setFormData({ ...formData, customBuildingFeatures: formData.customBuildingFeatures.filter(x => x !== f) });
  ...
  const handleSubmit = async (status: 'draft' | 'active') => {
    ...
    try {
      ...
      toast({ title: "Erro ao atualizar imóvel", description: "Tente novamente em alguns minutos.", variant: "destructive" });
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <DashboardLayout title="Editar Imóvel" userRole={dashboardRole}>
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <form className="space-y-6">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <Input id="zipcode" value={formData.zipcode} onChange={(e) => setFormData({...formData, zipcode: e.target.value})} placeholder="Ex: 88000-000" />
                </div>
                <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {(['sale', 'both'].includes(formData.purpose)) && (
                  <div>
                    ...
                    <Input id="price" type="number" value={formData.price} onChange={(e) => setFormData({...formData, price: e.target.value})} placeholder="Ex: 450000" />
                  </div>
                )}
                {(['rent', 'rent_annual', 'rent_seasonal', 'both'].includes(formData.purpose)) && (
                  <div>
                    <Label htmlFor="rental_price">Preço do Aluguel (R$) *</Label>
                    <Input id="rental_price" type="number" value={formData.rental_price} onChange={(e) => setFormData({...formData, rental_price: e.target.value})} placeholder="Ex: 2500" />
                  </div>
                )}
              </div>
            </CardContent>
          ...
          {/* Áreas e Medidas */}
          <Card>
            <CardHeader><CardTitle>Áreas e Medidas</CardTitle><CardDescription>Informações sobre as dimensões do imóvel</CardDescription></CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div><Label htmlFor="area">Área Privativa (m²)</Label><Input id="area" type="number" value={formData.area} onChange={(e) => setFormData({...formData, area: e.target.value})} placeholder="Ex: 85" /></div>
                <div><Label htmlFor="built_area">Área Construída (m²)</Label><Input id="built_area" type="number" value={formData.built_area} onChange={(e) => setFormData({...formData, built_area: e.target.value})} placeholder="Ex: 120" /></div>
                <div><Label htmlFor="total_area">Área Total (m²)</Label><Input id="total_area" type="number" value={formData.total_area} onChange={(e) => setFormData({...formData, total_area: e.target.value})} placeholder="Ex: 150" /></div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div><Label htmlFor="bedrooms">Quartos</Label><Input id="bedrooms" type="number" value={formData.bedrooms} onChange={(e) => setFormData({...formData, bedrooms: e.target.value})} placeholder="Ex: 3" /></div>
                <div><Label htmlFor="bathrooms">Banheiros</Label><Input id="bathrooms" type="number" value={formData.bathrooms} onChange={(e) => setFormData({...formData, bathrooms: e.target.value})} placeholder="Ex: 2" /></div>
                <div><Label htmlFor="lavabos">Lavabos</Label><Input id="lavabos" type="number" value={formData.lavabos} onChange={(e) => setFormData({...formData, lavabos: e.target.value})} placeholder="Ex: 1" /></div>
                <div><Label htmlFor="suites">Suítes</Label><Input id="suites" type="number" value={formData.suites} onChange={(e) => setFormData({...formData, suites: e.target.value})} placeholder="Ex: 1" /></div>
                <div><Label htmlFor="garage_spaces">Vagas Garagem</Label><Input id="garage_spaces" type="number" value={formData.garage_spaces} onChange={(e) => setFormData({...formData, garage_spaces: e.target.value})} placeholder="Ex: 2" /></div>
              </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div><Label htmlFor="development_name">Nome do Empreendimento</Label><Input id="development_name" value={formData.development_name} onChange={(e) => setFormData({...formData, development_name: e.target.value})} placeholder="Ex: Residencial Vista Mar" /></div>
                <div><Label htmlFor="apartment_number">Número do Apartamento</Label><Input id="apartment_number" value={formData.apartment_number} onChange={(e) => setFormData({...formData, apartment_number: e.target.value})} placeholder="Ex: 1201" /></div>
              </div>
              <div>
          ...
          <Card>
            <CardHeader><CardTitle>Detalhes Financeiros</CardTitle><CardDescription>Taxas e custos adicionais</CardDescription></CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div><Label htmlFor="condo_fee">Taxa de Condomínio (R$/mês)</Label><Input id="condo_fee" type="number" value={formData.condo_fee} onChange={(e) => setFormData({...formData, condo_fee: e.target.value})} placeholder="Ex: 350" /></div>
                <div><Label htmlFor="iptu_fee">IPTU (R$/ano)</Label><Input id="iptu_fee" type="number" value={formData.iptu_fee} onChange={(e) => setFormData({...formData, iptu_fee: e.target.value})} placeholder="Ex: 1200" /></div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader><CardTitle>Categorias Especiais</CardTitle><CardDescription>Marque as categorias que se aplicam</CardDescription></CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[
          ...
          {/* Tags */}
          <Card>
            <CardHeader><CardTitle>Tags</CardTitle><CardDescription>Selecione as tags que se aplicam a este imóvel</CardDescription></CardHeader>
            <CardContent>
          ...
          {/* Mídia e Tours */}
          <Card>
            <CardHeader><CardTitle>Mídia e Tours Virtuais</CardTitle><CardDescription>Adicione vídeos e tours virtuais</CardDescription></CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div><Label htmlFor="video_url">URL do Vídeo</Label><Input id="video_url" value={formData.video_url} onChange={(e) => setFormData({...formData, video_url: e.target.value})} placeholder="https://youtube.com/watch?v=..." /></div>
                <div><Label htmlFor="virtual_tour_url">URL do Tour Virtual</Label><Input id="virtual_tour_url" value={formData.virtual_tour_url} onChange={(e) => setFormData({...formData, virtual_tour_url: e.target.value})} placeholder="https://matterport.com/..." /></div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader><CardTitle>Informações do Corretor</CardTitle><CardDescription>Dados do corretor responsável</CardDescription></CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div><Label htmlFor="broker_name">Nome do Corretor</Label><Input id="broker_name" value={formData.broker_name} onChange={(e) => setFormData({...formData, broker_name: e.target.value})} placeholder="Nome completo" /></div>
                <div><Label htmlFor="broker_creci">CRECI</Label><Input id="broker_creci" value={formData.broker_creci} onChange={(e) => setFormData({...formData, broker_creci: e.target.value})} placeholder="Ex: CRECI 12345-J" /></div>
              </div>
              <div>
          ...
          <Card>
            <CardHeader><CardTitle>Dados do Proprietário</CardTitle><CardDescription>Informações confidenciais do proprietário (visível apenas para corretores e administradores)</CardDescription></CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div><Label htmlFor="owner_name">Nome do Proprietário</Label><Input id="owner_name" value={formData.owner_name} onChange={(e) => setFormData({...formData, owner_name: e.target.value})} placeholder="Nome completo do proprietário" /></div>
                <div><Label htmlFor="owner_phone">Telefone do Proprietário</Label><Input id="owner_phone" value={formData.owner_phone} onChange={(e) => setFormData({...formData, owner_phone: e.target.value})} placeholder="(47) 99999-9999" /></div>
              </div>
              <div><Label htmlFor="owner_email">Email do Proprietário</Label><Input id="owner_email" type="email" value={formData.owner_email} onChange={(e) => setFormData({...formData, owner_email: e.target.value})} placeholder="proprietario@email.com" /></div>
              <div>
          ...
          {/* Imagens */}
          <Card>
            <CardHeader><CardTitle>Imagens da Propriedade</CardTitle><CardDescription>Adicione fotos do imóvel (máximo 20 imagens). A marca d'água da logo será aplicada automaticamente.</CardDescription></CardHeader>
            <CardContent>
          ...
          <div className="flex justify-end space-x-4">
            ...
            <Button type="button" variant="secondary" disabled={isLoading} onClick={() => handleSubmit('draft')}>{isLoading ? 'Salvando...' : 'Salvar Rascunho'}</Button>
            <Button type="button" disabled={isLoading} onClick={() => handleSubmit('active')}>{isLoading ? 'Publicando...' : 'Publicar'}</Button>
          </div>
        </form>
      </div>
    </DashboardLayout>
